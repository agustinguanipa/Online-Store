<?php
  ...
  $id_admin=intval($_POST['restaurar_id']);
	
	// Borrado Logico de la Base de Datos FROM
    $sql = "UPDATE tabma_admi SET statu_admi = 1 WHERE ident_admi='$id_admin'";
    $query = mysqli_query($con,$sql);
    // Si el Borrado ha sido Exitoso
    if ($query) {
        $messages[] = "El administrador ha sido restaurado con éxito.";
    } else {
