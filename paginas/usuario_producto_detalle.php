<?php require_once('usuario_header.php'); ?>

<head>
	<title>Productos | SIGMAEMCA Online Store</title>
</head>

<body>
	<div class="jumbotron text-center" style="background-color: #FBFCFF;">
    <div class="container">
      <h1>Productos</h1>
      <hr class="my-4">
      <div class="row">
        <div class="col-sm-12 form-group">
          <div class="card">
            <div class="row no-gutters">
              <aside class="col-sm-5 border-right">
          <article class="gallery-wrap"> 
          <div class="img-big-wrap">
            <div> <a href="../imagen/productos/dinamapv100.jpg" data-fancybox=""><img src="../imagen/productos/dinamapv100.jpg"></a></div>
          </div> <!-- slider-product.// -->
          <div class="img-small-wrap">
            <div class="item-gallery"> <img src="../imagen/productos/dinamapv100.jpg"></div>
            <div class="item-gallery"> <img src="../imagen/productos/dinamapv100.jpg"></div>
            <div class="item-gallery"> <img src="../imagen/productos/dinamapv100.jpg"></div>
          </div> <!-- slider-nav.// -->
          </article> <!-- gallery-wrap .end// -->
              </aside>
              <aside class="col-sm-7">
          <article class="p-5">
            <h3 class="title mb-3">DINAMAP CARESCAPE VITAL SIGNS MONITOR V100</h3>

          <div class="mb-3"> 
            <var class="price h3 text-primary"> 
              <span class="currency">Bs.</span><span class="num">10.000.000,00</span>
            </var> 
            
          </div> <!-- price-detail-wrap .// -->
          <dl>
            <dt>Descripción</dt>
            <dd><p>Diseñado para Pacientes Adultos, Pediatricos y Neonatales. Es extremadamente preciso y confiable, no solo para la presión arterial, si no, para otro rango de parametros tambien.</p></dd>
          </dl>
          <dl class="row" align="left">
            <dt class="col-sm-3">Condición:</dt>
            <dd class="col-sm-9">Nuevo</dd>

            <dt class="col-sm-3">Peso:</dt>
            <dd class="col-sm-9">3.5 kg</dd>

            <dt class="col-sm-3">Tamaño:</dt>
            <dd class="col-sm-9">0.30 m x 0.30 m x 0.50 m</dd>
          </dl>
          <hr>
            <div class="row">
              <div class="col-sm-12">
                <dl class="dlist-inline">
                  <dt>Cantidad: </dt>
                  <dd> 
                    <select class="form-control form-control-sm" style="width:70px;">
                      <option> 1 </option>
                      <option> 2 </option>
                      <option> 3 </option>
                    </select>
                  </dd>
                </dl>  <!-- item-property .// -->
              </div> <!-- col.// -->
            </div> <!-- row.// -->
            <hr>
            <a href="usuario_cart.php" class="btn btn-primary float-center"><i class="fa fa-shopping-cart"></i> Agregar al Carrito</a> 
          </article> <!-- card-body.// -->
              </aside> <!-- col.// -->
            </div> <!-- row.// -->
          </div> <!-- card.// -->


        </div>
        
      </div>
    </div>
  </div>
</body>

<?php require_once('usuario_footer.php');  ?>

